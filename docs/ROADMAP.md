...
## Phase shift (2026-07-27): stop adding management docs, start proving value

User feedback: enough operating principles exist now - more markdown
policy has diminishing returns and risks becoming the over-engineering it
was meant to prevent. **No new principle/manifesto docs from here** -
docs/COMPANY_SCOREBOARD.md (auto-generated by `agents/scoreboard_agent.py`
every run, backed by the new `company_metrics` table) replaces "write
another doc" with real, computed numbers: asset counts, tier breakdown,
confirmed revenue (0 until real), and a run-over-run trend. A structural
`experiments` table exists too, deliberately unused by a scheduler until
there's real traffic to test against - building that harness now would
itself be the premature engineering being warned against.

Going forward, judge any new work by three questions, not by whether it
adds a new layer of process: **did it produce new knowledge, did it
